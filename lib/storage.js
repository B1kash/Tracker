...
// ============ CARDIO (Day-based) ============
// Structure: { id, date, type, duration, distance, calories, notes, completed }

export function getCardioLogs() {
  return getData(STORAGE_KEYS.cardio);
}

export function getCardioByDate(dateStr) {
  return getCardioLogs().filter((c) => c.date === dateStr);
}

export function addCardioLog(data) {
  ...
  logs.push(entry);
  saveData(STORAGE_KEYS.cardio, logs);
  return entry;
}

export function updateCardioLog(id, updates) {
  const logs = getCardioLogs();
  const idx = logs.findIndex((c) => c.id === id);
...
export function deleteCardioLog(id) {
  const logs = getCardioLogs().filter((c) => c.id !== id);
  saveData(STORAGE_KEYS.cardio, logs);
  return logs;
}

// ============ DIET (Day-based) ============
// Structure: { id, date, meal, food, calories, protein, carbs, notes }

export function getDietLogs() {
  return getData(STORAGE_KEYS.diet);
}

export function getDietByDate(dateStr) {
  return getDietLogs().filter((d) => d.date === dateStr);
}
...
export function addDietLog(data) {
  ...
  logs.push(entry);
  saveData(STORAGE_KEYS.diet, logs);
  return entry;
}

export function updateDietLog(id, updates) {
  const logs = getDietLogs();
  const idx = logs.findIndex((d) => d.id === id);
  if (idx === -1) return null;
  logs[idx] = { ...logs[idx], ...updates };
  saveData(STORAGE_KEYS.diet, logs);
  return logs[idx];
...
export function deleteDietLog(id) {
  const logs = getDietLogs().filter((d) => d.id !== id);
  saveData(STORAGE_KEYS.diet, logs);
  return logs;
}

// ============ CONTENT (Daily Logs) ============
// Structure: { id, title, platform, status: 'Created'|'Drafted'|'Posted', date, notes, createdAt, updatedAt }

export function getContentLogs() {
  return getData(STORAGE_KEYS.content);
}

export function getContentByDate(dateStr) {
  return getContentLogs().filter((c) => c.date === dateStr);
}

export function addContentLog(data) {
  const logs = getContentLogs();
  const newLog = {
    id: crypto.randomUUID(),
    ...data,
    createdAt: new Date().toISOString(),
  ...
  logs.push(newLog);
  saveData(STORAGE_KEYS.content, logs);
  return newLog;
}

export function updateContentLog(id, updates) {
  const logs = getContentLogs();
  const idx = logs.findIndex((c) => c.id === id);
  if (idx === -1) return null;
...
export function deleteContentLog(id) {
  const logs = getContentLogs().filter((c) => c.id !== id);
  saveData(STORAGE_KEYS.content, logs);
  return logs;
}

// ============ LEARNING (with Progress Logs) ============
// Structure: { id, title, resource, status, progress,
//   progressLogs: [{ id, date, whatLearned, doubts, keyTakeaways, timeSpent, mood }],
//   createdAt, updatedAt }

export function getLearningGoals() {
  return getData(STORAGE_KEYS.learning);
}

export function addLearningGoal(data) {
  ...
  goals.push(newGoal);
  saveData(STORAGE_KEYS.learning, goals);
  return newGoal;
}

export function updateLearningGoal(id, updates) {
  const goals = getLearningGoals();
...
export function deleteLearningGoal(id) {
  const goals = getLearningGoals().filter((g) => g.id !== id);
  saveData(STORAGE_KEYS.learning, goals);
  return goals;
}

export function addProgressLog(goalId, logData) {
  const goals = getLearningGoals();
  ...
  goals[idx].progressLogs = [log, ...(goals[idx].progressLogs || [])];
  goals[idx].updatedAt = new Date().toISOString();
  saveData(STORAGE_KEYS.learning, goals);
  return goals[idx];
}

export function deleteProgressLog(goalId, logId) {
  const goals = getLearningGoals();
  const idx = goals.findIndex((g) => g.id === goalId);
  if (idx === -1) return null;
  goals[idx].progressLogs = (goals[idx].progressLogs || []).filter((l) => l.id !== logId);
  goals[idx].updatedAt = new Date().toISOString();
  saveData(STORAGE_KEYS.learning, goals);
  return goals[idx];
}
...
export async function getStats() {
  ...
  const learning = getLearningGoals();
  const content = getContentLogs();
...
export async function getGymPhotos() {
  try { return await apiCall('/gym/photos'); } catch { return []; }
}

export async function addGymPhoto(dateStr, base64) {
  const res = await apiCall('/gym/photos', 'POST', { dateStr, base64 });
